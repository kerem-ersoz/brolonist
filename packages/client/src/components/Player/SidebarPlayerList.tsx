            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex-1 min-w-0 flex flex-col justify-center">
                <span className="text-white text-xs font-semibold truncate leading-tight">
                  {p.name}
                  {isMe && <span className="text-gray-400 ml-1">{t('player.you')}</span>}
                </span>
                <div className="flex items-center gap-1.5 mt-0.5">
                  <div className="flex items-center gap-0.5" title={p.hasLargestArmy ? t('player.largestArmy') : t('player.knightsPlayed')}>
                    ...
                    <span className={`text-[10px] font-medium ${p.hasLargestArmy ? 'text-yellow-400' : 'text-gray-300'}`}>{p.knightsPlayed}{p.hasLargestArmy && <span className="text-[9px]"> {t('player.largest')}</span>}</span>
                  </div>
                  <div className="flex items-center gap-0.5" title={p.hasLongestRoad ? t('player.longestRoad') : t('player.longestRoad')}>
                    <SpriteImage src={ICONS.road} fallback={<span className="text-[12px] leading-none">🛣️</span>} className="w-3 h-3 object-contain" />
                    <span className={`text-[10px] font-medium ${p.hasLongestRoad ? 'text-yellow-400' : 'text-gray-300'}`}>{p.longestRoadLength ?? 0}{p.hasLongestRoad && <span className="text-[9px]"> {t('player.longest')}</span>}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-1.5 flex-shrink-0">
                <div className="relative" title={t('player.resourceCards')}>
                ...
                <div className="relative" title={t('player.developmentCards')}>
                ...
                <div className="flex flex-col items-end gap-0 ml-1">
                  <div className="flex items-center gap-0.5">
                    ...
                    <span className="text-[10px] text-gray-400">{t('player.vp')}</span>
                  </div>
                  {isMe && (() => {
                    const vpCards = (p.developmentCards as Array<{ type: string }>).filter(c => c.type === 'victory_point').length;
                    if (vpCards > 0) {
                      const publicVP = p.victoryPoints - vpCards;
                      return (
